...
{	
	...
	public class CorgiController : MonoBehaviour 
	{
		...
		public virtual bool CanGoBackToOriginalSize()
		{
			...
			bool headCheckLeft = MMDebug.RayCast(_originalSizeRaycastOrigin, transform.up, headCheckDistance, PlatformMask - OneWayPlatformMask, Colors.LightSlateGray, true);

			_originalSizeRaycastOrigin = _boundsTopRightCorner + (Vector2)transform.up * _smallValue;
			bool headCheckRight = MMDebug.RayCast(_originalSizeRaycastOrigin, transform.up, headCheckDistance, PlatformMask - OneWayPlatformMask, Colors.LightSlateGray, true);
